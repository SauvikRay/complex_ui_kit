...
const Color _kDialogColor = CupertinoDynamicColor.withBrightness(
  color: Color(0xFFFFFFFF), // Color(0xCCF2F2F2),
...
class _RenderCupertinoDialog extends RenderBox {
  ...
  void performLayout() {
    ...
    assert(
      (!isActionSheet && actionsSection!.parentData is BoxParentData) || (isActionSheet && actionsSection!.parentData is MultiChildLayoutParentData),
    );
    if (isActionSheet) {
  ...
  _AlertDialogSizes performRegularLayout({required BoxConstraints constraints, required ChildLayouter layoutChild}) {
    final bool hasDivider = contentSection!.getMaxIntrinsicHeight(computeMaxIntrinsicWidth(0)) > 0.0 && actionsSection!.getMaxIntrinsicHeight(computeMaxIntrinsicWidth(0)) > 0.0;
    ...
    return _AlertDialogSizes(
      size: isActionSheet ? Size(constraints.maxWidth, dialogHeight) : constraints.constrain(Size(_dialogWidth, dialogHeight)),
  ...
  _AlertDialogSizes performAccessibilityLayout({required BoxConstraints constraints, required ChildLayouter layoutChild}) {
    final bool hasDivider = contentSection!.getMaxIntrinsicHeight(_dialogWidth) > 0.0 && actionsSection!.getMaxIntrinsicHeight(_dialogWidth) > 0.0;
    final double dividerThickness = hasDivider ? _dividerThickness : 0.0;
...
class CupertinoDialogAction extends StatelessWidget {
  ...
  double _calculatePadding(BuildContext context) {
    return 8.0 * MediaQuery.textScalerOf(context).scale(1.0);
  }
...
class _RenderCupertinoDialogActions extends RenderBox with ContainerRenderObjectMixin<RenderBox, MultiChildLayoutParentData>, RenderBoxContainerDefaultsMixin<RenderBox, MultiChildLayoutParentData> {
  ...
  bool _isSingleButtonRow(double width) {
    ...
    if (childCount == 1) {
      ...
      final double sideBySideWidth = firstChild!.getMaxIntrinsicWidth(double.infinity) + dividerThickness + lastChild!.getMaxIntrinsicWidth(double.infinity);
...
class CupertinoCustomAlertDialog extends StatefulWidget {
  /// Creates an iOS-style alert dialog.
  const CupertinoCustomAlertDialog({
    super.key,
    this.title,
    this.content,
  ...
  @override
  State<CupertinoCustomAlertDialog> createState() => _CupertinoCustomAlertDialogState();
}

class _CupertinoCustomAlertDialogState extends State<CupertinoCustomAlertDialog> {
  ScrollController? _backupScrollController;

  ScrollController? _backupActionScrollController;

  ScrollController get _effectiveScrollController => widget.scrollController ?? (_backupScrollController ??= ScrollController());

  ScrollController get _effectiveActionScrollController => widget.actionScrollController ?? (_backupActionScrollController ??= ScrollController());
